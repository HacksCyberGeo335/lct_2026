...
```bash
retro0@marinas storage_sys % docker compose -f docker-compose.yaml exec postgres psql -U prom_app -d prom_app -c "SELECT 1 AS db_is_alive;"
 db_is_alive 
-------------
           1
(1 row)

retro0@marinas storage_sys % docker compose -f docker-compose.yaml exec postgres psql -U prom_app -d prom_app -c "SELECT * from general_video_table"                                  
 uuid | video_name | storage_key | status | created_at | updated_at 
------+------------+-------------+--------+------------+------------
(0 rows)

retro0@marinas storage_sys % docker compose -f docker-compose.yaml exec postgres psql -U prom_app -d prom_app -c "SELECT * from meta_video_table"   
