...
В этой папке лежит документация по локальному MinIO. Общий `docker-compose.yml` для инфраструктуры проекта находится в корне репозитория.
...
```bash
cd /Users/retro0/projects/programming/PyPrj/hackatons/etc/0_lct_2026/lct_2026
```
...
```bash
docker run --rm --network lct_2026_default --entrypoint /bin/sh minio/mc:latest -c 'mc alias set local http://s3-storage:9000 prom_app prom_app_password >/dev/null && mc ls local'
```
